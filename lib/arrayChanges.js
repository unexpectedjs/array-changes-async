    ...
    arrayDiff([].concat(actual), [].concat(expected), function (a, b, aIndex, bIndex, callback) {
        ...
        countConflicts(0, 0, MAX_STACK_DEPTH, function (c) {
            ...
            if (includeNonNumericalProperties) {
                ...
                var addNonNumericalKey = function (i, stackCallsRemaining, callback) {
                    if (i < nonNumericalKeys.length) {
                        var key = nonNumericalKeys[i];
                        if (key in actual) {
                            if (key in expected) {
                                return equal(actual[key], expected[key], key, key, function (areEqual) {
                                    mutatedArray.push({
                                        type: areEqual ? 'equal' : 'similar',
                                        expectedIndex: key,
                                        actualIndex: key,
                                        value: actual[key],
                                    ...
                                    if (stackCallsRemaining === 0) {
                                        return setTimeout(function () {
                                            addNonNumericalKey(i + 1, MAX_STACK_DEPTH, callback);
                                        })
                                    }
                                    addNonNumericalKey(i + 1, stackCallsRemaining - 1, callback);
                                });
                            } else {
                                mutatedArray.push({
                                    type: 'remove',
                ...
                addNonNumericalKey(0, MAX_STACK_DEPTH, function () {
